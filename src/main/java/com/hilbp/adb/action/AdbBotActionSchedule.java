...
@Slf4j
@Component
public class AdbBotActionSchedule implements ActionSchedule {
	...
	public void run(JadbDevice device, List<Action> actions) {
		...
		while(true) {
			...
			if(RunningStatus.isFinished(device) == false) {
				try {
				...
				this.runFinishedAction(device, actions);
				break;
			}
		}
	}
	...
	//执行status=finished状态的action（只执行一次）
	public void runFinishedAction(JadbDevice device, List<Action> actions) {

		List<Action> finishedActions = this.filterAction(actions, StaticValue.STATUS_FINISHED);
		if(finishedActions == null || finishedActions.isEmpty())
			return;
	...
	public void runRunningAction(JadbDevice device, List<Action> actions) {
		...
		while(RunningStatus.isFinished(device) == false) {
	...
	public void runChildActionOfParentTypeIsClickTargetNode(JadbDevice device, List<Action> childActions) {
		...
		outLoop: while(i > 0) {
			for(Action childAction : childActions) {
				if(RunningStatus.isAbortSubsteps(device) == false) {
					this.invoke(device, childAction);
				} else {
					RunningStatus.clearAbortSubsteps(device);
					break outLoop;
				}
			}
			i--;
	...
	public void invoke(JadbDevice device, Action action) {
		...
		AbstractActionType type = (AbstractActionType) applicationContext.getBean(action.getType());
		type.start(device, action);
	}
